...
import {
  ...
  Activity, Percent, Megaphone, Store as StoreIcon,
} from "lucide-react";
  ...
  return (
    <AppShell>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        <Kpi icon={TrendingUp} label="Toplam forecast" value={fmtNum(totForecast)} hint="Günlük plan" />
        <Kpi icon={Activity} label="Gerçekleşen" value={fmtNum(totActual)} hint={`Gün sonu: ${fmtNum(totEod)}`} />
        <Kpi icon={TrendingDown} label="Gap" value={fmtPct(gap)} tone={gap < -5 ? "danger" : gap < 0 ? "warning" : "success"} />
        <Kpi icon={Receipt} label="Beklenen ek sipariş" value={`+${fmtNum(totIncOrders)}`} tone="success" />
        <Kpi icon={Wallet} label="Beklenen ek ciro" value={fmtMoney(totIncRev)} tone="success" />
      </div>

      <div className="mt-3 grid grid-cols-2 gap-3 lg:grid-cols-5">
        <Kpi icon={Percent} label="Ortalama CR" value={`%${avgCr.toFixed(2)}`} />
        <Kpi icon={TrendingUp} label="Ortalama ROI" value={avgRoi > 0 ? `${avgRoi.toFixed(1)}x` : "—"} />
        <Kpi icon={ShieldCheck} label="GREEN mağaza" value={String(green)} tone="success" />
        <Kpi icon={ShieldAlert} label="YELLOW mağaza" value={String(yellow)} tone="warning" />
        <Kpi icon={ShieldX} label="RED mağaza" value={String(red)} tone="danger" />
      </div>
            ...
            <tbody>
              {filtered.map((r) => {
                const rowTone =
                  ...
                  : "hover:bg-accent/40";
                return (
                  <tr key={r.store} className={`border-t border-border ${rowTone}`}>
